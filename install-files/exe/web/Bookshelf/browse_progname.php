...
//	include_once('includes/authentication.inc.php');
	...
	$dbquery= "Select TrajId from TrajData where ProgramCode = '$keyword'";
	...
	$query= "Select TrajId,UserName,DATE_FORMAT(Date,'%d/%m/%y')as Date,ProteinName,ProgramCode,UserComments,PmfOption,Doi,Pid, Date As sortdate from TrajData where ProgramCode = '$keyword' ORDER BY sortdate DESC LIMIT $page, $limit";
	$result= @mysqli_query($link,$query); //run the query
	
	echo '<h3 class= "title"> Browse Database: Program Name - '.$keyword.'</h3>';
//start of top page bar
	...
	if ($numrows>20){
		...
		if ($page != 0){ // Don't show back link if current page is first page.				
			echo'<a href="browse_progname.php?keyword='.$keyword.'&page='.$firstpage.'&limit='.$limit.'&numrows='.$numrows.'"><b> First| </b></a>';
			$back_page = $page - $limit;
			echo'<a href="browse_progname.php?keyword='.$keyword.'&page='.$back_page.'&limit='.$limit.'&numrows='.$numrows.'"><b> Prev| </b></a>';}
			echo ' Page <b>'.$current.'</b> of <b>'.$total.'</b>';
		if (!((($page+$limit) / $limit) >= $pages) && $pages != 1) { // If last page don't give next link.
			$next_page = $page + $limit;
			$lastpage=$numrows-($numrows%$limit);
			echo'<a href="browse_progname.php?keyword='.$keyword.'&page='.$next_page.'&limit='.$limit.'&numrows='.$numrows.'"><b> |Next</b></a>';
			echo'<a href="browse_progname.php?keyword='.$keyword.'&page='.$lastpage.'&limit='.$limit.'&numrows='.$numrows.'"><b> |Last</b></a>';}	
		else{
			echo'|Next|Last';}
		if ($limit==$numrows){
			$limit=20;
			echo'<td align="right"> <a href="browse_progname.php?keyword='.$keyword.'&page='.$firstpage.'&limit='.$limit.'&numrows='.$numrows.' "> <u>Show page</u></a>';}
		else{
			echo'<td align="right"> <a href="browse_progname.php?keyword='.$keyword.'&page='.$firstpage.'&limit='.$numrows.'&numrows='.$numrows.' "> <u>Show all</u></a>';}
	}
	echo'</td></tr></table>';
//end of top page bar
//start of Data table
	...
	if ($numrows>20){
		...
		for ($i=1; $i <= $pages; $i++)// loop through each page and give link to it.
		{
			...
			elseif ($ppage < $page - $limitnum || $ppage > $page + $limitnum){}
					...
					echo'<a href="browse_progname.php?keyword='.$keyword.'&page='.$ppage.'&limit='.$limit.'&numrows='.$numrows.'">'.$i.'</a> ';}
		}//end of for loop
		if (!((($page+$limit) / $limit) >= $pages) && $pages != 1) { // If last page don't give next link.
					$next_page = $page + $limit;
					echo'<a href="browse_progname.php?keyword='.$keyword.'&page='.$next_page.'&limit='.$limit.'&numrows='.$numrows.'"><b>|| Next</b></a>';
