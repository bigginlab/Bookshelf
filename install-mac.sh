...
check_User()
{
	...
	new_user="$(/usr/bin/dscl . -search /Users name "$username" 2>/dev/null)"
	if [[ -z "$new_user" ]]; then
		flag="false"
...
############## Create MySql user to read and write to bookshelf database ###############
	...
	pass=$(head -c8 /dev/urandom | uuencode -m - | sed -n 2p |sed -e 's/[^[:alnum:]|-]//g')
...
############## Create login user only to read and write the login database ###############
	...
	echo "Password for BS.login $loginpass"
 	loginpass=$(head -c8 /dev/urandom | uuencode -m - | sed -n 2p | sed -e 's/[^[:alnum:]|-]//g')
...
############## Create users and database ###############
	...
	homeDir="defaultpath"
	sed -ie  "s%${homeDir}%${bpth}%g" ./exe/bin/bookshelf.daemon.py 
	sed -ie  "s%${homeDir}%${bpth}%g" ./exe/bin/bookshelf.erase.py 
	sed -ie  "s%${homeDir}%${bpth}%g" ./exe/bs/bookshelf.deposition.py 
	chmod 777 ./bookshelf	
	...
	if [ "$newtempFolder" == "" ]; then
		...
		sed -ie  "s%${tempFolder}%${newtempFolder}%g" ./exe/bin/bookshelf.daemon.py 
		sed -ie  "s%${tempFolder}%${newtempFolder}%g" ./exe/bs/bookshelf.deposition.py 
	...
	if [ "$newtrajFolder" == "" ]; then
		...
		sed -ie  "s%${trajFolder}%${newtrajFolder}%g" ./exe/bin/bookshelf.daemon.py 
		sed -ie  "s%${trajFolder}%${newtrajFolder}%g" ./exe/bin/bookshelf.erase.py 
		...
		sed -ie  "s%${junkFolder}%${newerrorFolder}%g" ./exe/bin/bookshelf.daemon.py 
	fi
############## Creating Bookshelf administrator ##############
	...
	if [ "$flag" == "false" ];then
		...
		declare -i new_uid=0 i=500 idvar=0

		while [[ $idvar -eq 0 ]]; do 
		   i=$[i+1]
		   if [[ -z "$(/usr/bin/dscl . -search /Users uid $i)" ]] ; then
		      new_uid=$i
		      idvar=1
		      #break
		   fi
		...
		if [[ $new_uid -eq 0 ]] ; then printf 'Getting unique id number (uid) failed!\n'; exit 1; fi
...
############## changing permissions ##############
	rm ./exe/bin/*.pye
	rm ./exe/bs/*.pye
	rm ./exe/web/Bookshelf/includes/*.phpe
	chmod -R 700 ./exe/bin/
